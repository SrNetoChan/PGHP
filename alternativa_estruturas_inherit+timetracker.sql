...
-- criar indices para a class, para o gid
...
$$
SELECT "gid","uniterr_oid","classe","tipo","nome","cadeado","estado","accao","observacoes","geom","time_start","user_update" FROM "PGHP"."infraestruturas" WHERE
...
BEGIN
  ...
  IF NEW."time_end" IS NULL THEN
    ...
    IF var_geomtype IN ('MULTIPOLYGON', 'POLYGON') THEN
      INSERT INTO "PGHP"."infraestruturas_poligonos" ("gid","uniterr_oid","classe","tipo","nome","cadeado","estado","accao","observacoes","geom","time_start","time_end","user_update")
      VALUES (OLD."gid",OLD."uniterr_oid",OLD."classe",OLD."tipo",OLD."nome",OLD."cadeado",OLD."estado",OLD."accao",OLD."observacoes",OLD."geom",OLD."time_start",current_timestamp,user);
    ELSIF var_geomtype IN ('LINESTRING', 'MULTILINESTRING') THEN
      INSERT INTO "PGHP"."infraestruturas_linhas" ("gid","uniterr_oid","classe","tipo","nome","cadeado","estado","accao","observacoes","geom","time_start","time_end","user_update")
      VALUES (OLD."gid",OLD."uniterr_oid",OLD."classe",OLD."tipo",OLD."nome",OLD."cadeado",OLD."estado",OLD."accao",OLD."observacoes",OLD."geom",OLD."time_start",current_timestamp,user);
    ELSE
      INSERT INTO "PGHP"."infraestruturas_pontos" ("gid","uniterr_oid","classe","tipo","nome","cadeado","estado","accao","observacoes","geom","time_start","time_end","user_update")
      VALUES (OLD."gid",OLD."uniterr_oid",OLD."classe",OLD."tipo",OLD."nome",OLD."cadeado",OLD."estado",OLD."accao",OLD."observacoes",OLD."geom",OLD."time_start",current_timestamp,user);
    END IF;
...
-- REVER PARA CONSIDERAR a alteração do gid
...
SELECT	"gid", "uniterr_oid", "tipo", "nome", "cadeado", "estado", "accao", "observacoes", "geom"::Geometry(POINT, 3763), "time_start", "user_update"
...
CREATE OR REPLACE RULE "_INSERT" AS ON INSERT TO "PGHP".infra_portoes DO INSTEAD
  INSERT INTO "PGHP"."infraestruturas_pontos" ("gid","uniterr_oid","classe","tipo","nome","cadeado","estado","accao","observacoes","geom")
    VALUES (NEW."gid",NEW."uniterr_oid",'portoes',NEW."tipo",NEW."nome",NEW."cadeado",NEW."estado",NEW."accao",NEW."observacoes",NEW."geom");
CREATE OR REPLACE RULE "_UPDATE" AS ON UPDATE TO "PGHP".infra_portoes DO INSTEAD
  UPDATE "PGHP"."infraestruturas_pontos" SET "gid" = NEW."gid","uniterr_oid" = NEW."uniterr_oid","classe" = 'portoes',"tipo" = NEW."tipo","nome" = NEW."nome","cadeado" = NEW."cadeado","estado" = NEW."estado","accao" = NEW."accao","observacoes" = NEW."observacoes","geom" = NEW."geom" WHERE gid = NEW.gid;
